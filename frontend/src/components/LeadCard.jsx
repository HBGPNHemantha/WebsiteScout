...
export default function LeadCard({ business }) {
  ...
  return (
    <div
      ...
      className={`group relative rounded-xl border transition-all duration-200 p-4 sm:p-5 ${
        business.hasWebsite
          ...
          : 'bg-slate-900/90 border-slate-800 hover:border-indigo-500/50 hover:shadow-xl hover:shadow-indigo-500/5'
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start space-x-3">
          <input
            ...
            className="mt-1 rounded bg-slate-950 border-slate-700 text-indigo-600 focus:ring-indigo-500/20 w-4 h-4 cursor-pointer"
          />
          <div>
            <h3
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <span className="text-[11px] font-medium px-2 py-0.5 rounded bg-slate-800 text-slate-300 capitalize border border-slate-700/60">
                {business.category}
              </span>
              ...
              {business.rating > 0 ? (
                <span className="flex items-center space-x-1 text-xs text-amber-400 font-semibold">
                  ...
                  <span className="text-slate-500 font-normal">({business.totalRatings || 0})</span>
                </span>
              ) : (
                <span className="text-[11px] text-slate-500">No reviews yet</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div>
          {business.hasWebsite ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex items-center space-x-1 text-[11px] font-semibold px-2 py-1 rounded-md bg-slate-800/80 text-slate-400 hover:text-slate-200 border border-slate-700 transition"
            ...
            <span className="inline-flex items-center space-x-1 text-[11px] font-extrabold px-2.5 py-1 rounded-md bg-rose-500/20 text-rose-300 border border-rose-500/40 animate-pulse">
      ...
      <div className="mt-4 pt-3 border-t border-slate-800/80 grid grid-cols-2 sm:grid-cols-3 gap-2">
        ...
        {telLink ? (
          <a
            ...
            className="flex items-center justify-center space-x-1.5 py-2 px-3 rounded-lg bg-slate-800/90 hover:bg-slate-700 text-slate-200 text-xs font-semibold border border-slate-700 transition"
          ...
          <div className="flex items-center justify-center space-x-1.5 py-2 px-3 rounded-lg bg-slate-900 text-slate-600 text-xs font-medium border border-slate-800 cursor-not-allowed">
        ...
        {whatsappLink ? (
          <a
            ...
            className="flex items-center justify-center space-x-1.5 py-2 px-3 rounded-lg bg-emerald-950/60 hover:bg-emerald-900/60 text-emerald-300 text-xs font-semibold border border-emerald-800/60 transition"
        ...
        <button
          ...
          className="col-span-2 sm:col-span-1 flex items-center justify-center space-x-1.5 py-2 px-3 rounded-lg bg-indigo-950/60 hover:bg-indigo-900/60 text-indigo-300 text-xs font-semibold border border-indigo-800/60 transition"
      ...
      <div className="mt-3 pt-3 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="relative">
          ...
          {showStatusMenu && (
            <div className="absolute left-0 bottom-full mb-1 w-44 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-20 p-1 space-y-0.5">
        ...
        <div className="flex items-center space-x-1">
          ...
          <button
            ...
            className="p-1.5 rounded text-slate-400 hover:text-indigo-300 hover:bg-slate-800 text-xs transition"
            title="Focus on Map"
          >
          ...
          <button
            ...
            className="p-1.5 rounded text-slate-400 hover:text-rose-400 hover:bg-slate-800 text-xs transition"
            title="Delete lead"
          >
      ...
      {/* Inline Notes Editor */}
      {isEditingNotes && (
        <div className="mt-3 p-2 bg-slate-950 rounded-lg border border-slate-800 space-y-2 animate-fadeIn">
          <textarea
            value={noteText}
            onChange={(e) => setNoteText(e.target.value)}
            placeholder="Add notes (e.g. Spoke with owner John, wants 5-page quote by Tuesday)..."
            rows={2}
            className="w-full p-2 bg-slate-900 border border-slate-700 rounded-md text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500"
          />
          <div className="flex justify-end space-x-2">
            ...
            <button
              ...
              className="px-2.5 py-1 text-[11px] bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-md"
            >
              Save Note
            </button>
          </div>
      ...
      {!isEditingNotes && business.notes && (
        <div className="mt-2 text-[11px] text-slate-300 bg-slate-950/70 px-2.5 py-1.5 rounded-lg border border-slate-800/80 italic line-clamp-2">
          "{business.notes}"
        </div>
      )}
