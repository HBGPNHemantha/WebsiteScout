...
export default function SearchBar() {
  ...
  return (
    <div className="w-full bg-gradient-to-b from-slate-900/90 to-slate-950/90 border border-slate-800/80 rounded-2xl p-4 sm:p-6 shadow-2xl relative overflow-hidden backdrop-blur-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3">
          ...
          <div className="md:col-span-5 relative">
            ...
            <div className="relative flex items-center">
              <Search className="absolute left-3.5 w-5 h-5 text-indigo-400 pointer-events-none" />
              <input
                type="text"
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                placeholder="e.g. bookshops, dentists, plumbers..."
                className="w-full pl-11 pr-4 py-3 bg-slate-950/90 border border-slate-700/80 hover:border-slate-600 focus:border-indigo-500 rounded-xl text-slate-100 placeholder-slate-500 font-medium text-sm transition shadow-inner focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                required
              />
            </div>
          </div>
          ...
          <div className="md:col-span-5 relative">
            ...
            <div className="relative flex items-center">
              <MapPin className="absolute left-3.5 w-5 h-5 text-rose-400 pointer-events-none" />
              <input
                type="text"
                value={area}
            ...
            {showRecent && recentSearches.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-30 p-2 space-y-1">
                <div className="text-[11px] text-slate-400 px-2 py-1 font-semibold uppercase tracking-wider border-b border-slate-800">
                ...
                {recentSearches.map((s, idx) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-indigo-600 hover:text-white rounded-lg flex items-center justify-between transition"
                  >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-800/80">
          ...
          <label className="flex items-center space-x-2 cursor-pointer text-xs text-slate-400 hover:text-slate-300">
            <input
              ...
              className="rounded bg-slate-900 border-slate-700 text-indigo-600 focus:ring-indigo-500/20 w-3.5 h-3.5"
            />
