...
export default function MapView() {
  ...
  return (
    <div className="w-full h-full min-h-[450px] relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl bg-slate-100 dark:bg-slate-950 transition-colors">
      
      {/* Map Legend Overlay */}
      <div className="absolute top-3 right-3 z-[1000] bg-white/90 dark:bg-slate-950/85 backdrop-blur-md border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 shadow-xl text-[11px] space-y-1.5 hidden sm:block">
        <div className="font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-[10px] pb-1 border-b border-slate-200 dark:border-slate-800">
          Pin Legend
        </div>
          ...
          return (
            ...
            >
              <Popup className="websitescout-map-popup">
                <div className="p-3.5 max-w-[260px] text-slate-800 dark:text-slate-100 bg-white dark:bg-slate-900">
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 capitalize border border-slate-200 dark:border-slate-700">
                      {business.category}
                    </span>
                    {business.rating > 0 && (
                      <span className="flex items-center text-[11px] text-amber-500 dark:text-amber-400 font-semibold">
                  ...
                  <div className="mt-2">
                    {business.hasWebsite ? (
                      ...
                      <span className="text-[10px] font-extrabold px-2 py-0.5 rounded bg-rose-50 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-500/30">
                        NO WEBSITE (HOT LEAD)
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-2 text-[11px] text-slate-500 dark:text-slate-400 space-y-1">
                    ...
                    {business.phone && (
                      <p className="font-semibold text-slate-700 dark:text-slate-300">{business.phone}</p>
                    )}
                  </div>

                  {/* Quick Action Buttons */}
                  <div className="mt-3 pt-2 border-t border-slate-100 dark:border-slate-800 grid grid-cols-2 gap-1.5">
                    {whatsappLink && (
                      <a
                        href={whatsappLink}
                        target="_blank"
                        ...
                        className="flex items-center justify-center space-x-1 py-1.5 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/80 dark:hover:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold rounded border border-emerald-200 dark:border-emerald-800/60"
                      >
                    ...
                    {telLink && (
                      <a
                        ...
                        className="flex items-center justify-center space-x-1 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-[10px] font-bold rounded border border-slate-200 dark:border-slate-700"
                      >
                        <Phone className="w-3 h-3 text-blue-500 dark:text-blue-400" />
                        <span>Call</span>
                      </a>
                    )}
                  ...
                  <div className="mt-2">
                    <select
                      ...
                      className={`w-full text-[10px] font-bold py-1 px-2 rounded border bg-white dark:bg-slate-950 ${statusConfig.bgLight}`}
                    >
                      {Object.values(PIPELINE_STATUSES).map((st) => (
                        <option
                          key={st.key}
