  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 shadow-xl relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Leads Scouted</span>
            <div className="w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-400 flex items-center justify-center">
              <Users className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-slate-100">{summary.totalLeads}</span>
            <span className="text-xs text-indigo-400 font-semibold">{summary.noWebsiteCount} without site</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400">
            <strong className="text-slate-200">{summary.noWebsiteRate}%</strong> qualifying rate
          </div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 shadow-xl relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Outreach Progress</span>
            <div className="w-8 h-8 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center">
              <PhoneCall className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-blue-400">{summary.outreachRate}%</span>
            <span className="text-xs text-slate-400">({summary.contactedTotal} contacted)</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 shadow-xl relative overflow-hidden">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-emerald-400">{summary.convertedCount}</span>
            <span className="text-xs text-emerald-400/80 font-semibold">{summary.conversionRate}% win rate</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-400">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 shadow-xl relative overflow-hidden">
          ...
          <div className="mt-3 flex items-baseline space-x-2">
            ...
            <span className="text-xs text-slate-400">Est. Deals</span>
          </div>
      ...
      <div className="p-6 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-extrabold text-slate-100">Sales Pipeline Stages</h2>
            <p className="text-xs text-slate-400">Click any stage to filter leads in Scout view</p>
          </div>
        </div>
          ...
          {Object.values(PIPELINE_STATUSES).map((st) => {
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border cursor-pointer transition-all hover:scale-[1.02] ${st.bgLight} hover:shadow-lg`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-base font-bold text-slate-100">Top Scouted Locations</h2>
          </div>

          <div className="space-y-2">
            {topAreas.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 text-center">No location data yet</p>
            ) : (
              topAreas.map((item, idx) => (
                <div
                  ...
                  className="flex items-center justify-between p-3 bg-slate-950/70 hover:bg-slate-800/80 rounded-xl border border-slate-800/80 cursor-pointer transition"
                >
                  <div className="flex items-center space-x-3">
                    <span className="text-xs font-bold text-slate-500 w-4">{idx + 1}</span>
                    <span className="text-xs font-semibold text-slate-200">{item.area}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <span className="text-xs font-extrabold text-rose-400">
                      {item.noWebsites} Leads
                    </span>
                    <span className="text-[11px] text-slate-500">({item.count} total)</span>
        ...
        <div className="p-6 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-base font-bold text-slate-100">High-Opportunity Niches</h2>
          </div>

          <div className="space-y-2">
            {topCategories.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 text-center">No category data yet</p>
            ) : (
      ...
      <div className="p-6 bg-slate-900/90 border border-slate-800 rounded-2xl shadow-xl space-y-4">
        <h2 className="text-base font-bold text-slate-100">Recent Search Activity</h2>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {recentSearches.map((s, idx) => (
                <tr key={idx} className="hover:bg-slate-800/40 transition">
                  <td className="p-3 font-semibold text-slate-200 capitalize">{s.category}</td>
                  <td className="p-3 text-slate-300">{s.area}</td>
                  <td className="p-3">
                    <span className="px-2 py-0.5 rounded bg-rose-500/20 text-rose-300 font-extrabold text-[11px] border border-rose-500/30">
                      {s.noWebsiteCount} leads
                    </span>
                  </td>
                  <td className="p-3 text-slate-400">{s.totalFound}</td>
                  ...
                  <td className="p-3 text-right">
                    <button
                      ...
                      className="px-3 py-1 bg-slate-800 hover:bg-indigo-600 hover:text-white text-slate-300 text-xs font-semibold rounded-lg transition"
                    >
                      Re-Scout
                    </button>
                  </td>
